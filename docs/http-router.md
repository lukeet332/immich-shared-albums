...
| `web/passthrough.rs` | The transparent fall-through proxy to Immich for anything that isn't a sidecar route (SPA bundles, `/api`, `?native=1` share pages). Pure stream both directions — it buffers and rewrites nothing; uploads must never be buffered here. The upstream client bounds connection (10s) and response-header arrival plus mid-stream silence (30s idle) so a hung Immich fails loudly; transfers themselves are unbounded. |
...
| `web/route_error.rs` | `RouteError` — why an operation failed, and therefore the status: `BadInput` answers 400 (the caller's request was wrong; retrying unchanged is pointless), `Unavailable` answers 502 (Immich, the transport, or our own mid-retry state did not answer; retrying is right). Both answer the same `{"error": …}` body the panel parses. `join`'s `Refused` carries the same split as `retryable`. |
