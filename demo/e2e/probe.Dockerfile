...
#
# The build CONTEXT is staged by run-mock-e2e.sh (rust/target/probe-context) rather than the repo
# root: a root `docker build .` walks demo/ and fails merely STAT-ing the rig's root-owned data dirs.
WORKDIR /app
