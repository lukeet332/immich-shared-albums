...
| Rig | cross-household behaviour against live mock Immich stacks, with the Rust image as all three sidecars | `ISA_DOCKERFILE=rust/Dockerfile bash demo/run-mock-e2e.sh` | `ALL PASS (251 checks)` — 23 more than the TypeScript lane runs, the Rust-only gates (`native leave`, `unlink`'s ledger cleanup, `store-shared-locally`) among them |
| Browser | the banner, the accept page, the chooser, the settings card and the panel's live flows in Chromium | `cd demo/e2e && CKEY=… B_EMAIL=admin@e2e.local B_PASS=… node browser-test.mjs` | `BROWSER PASS (55 checks)` (the lane skips one check when C is not password-hardened) |
