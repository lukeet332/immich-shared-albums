...
fn main() {
    ...
    println!("kv(identity) readable: {:?}", store.kv("identity").map(|v| v.is_some()));
    let raw = store.kv("identity").ok().flatten();
    if let Some(v) = &raw {
        println!("identity row: v={} alg={} keys={:?}",
    ...
    match &state.identity {
        Some(id) => println!("parsed identity: pub={} priv_len={} createdAt={}",
            &id.public[..id.public.len().min(12)], id.private.len(), id.created_at),
        None => println!("parsed identity: NONE — a boot here would MINT A NEW KEY"),
    }
    println!("loaded: peers={} mappings={} contributors={}",
        state.peers.len(), state.mappings.len(), state.contributors.len());
}
