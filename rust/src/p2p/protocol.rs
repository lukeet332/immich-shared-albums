...
pub fn ledger_of_state() -> Ledger<'static> {
    ...
    Ledger {
        ...
        has_ledger_row: &|id: &str| {
            state().store.ledger_by_asset(id).ok().flatten().is_some()
        },
    }
}
...
pub async fn handle_redeem(caller_pub: &str, body: &[u8]) -> (u16, Value) {
    ...
    let share_key = parsed.get("shareKey").and_then(|k| k.as_str()).unwrap_or_default().to_string();
    ...
    let Some(link) = get_shared_link_by_key(&client, &share_key).await else {
        return (404, json!({ "error": "unknown share key", "code": "unknown_share_key" }));
    };
    ...
    if let Some(expires_at) = link.get("expiresAt").and_then(|e| e.as_str()) {
        if let Ok(expiry) = chrono_parse_ms(expires_at) {
            if expiry <= now_ms() {
                ...
                return (403, json!({ "error": "this share link has expired", "code": "link_expired" }));
            }
        }
    }
    ...
    match link.get("password").and_then(|p| p.as_str()) {
        Some(expected) if !expected.is_empty() => match password {
            ...
            Some(given) if !secret_equals(expected, given) => {
                ...
                return (403, json!({ "error": "incorrect album password", "code": "wrong_password" }));
            }
            _ => {}
        },
    ...
    let album_id = link.pointer("/album/id").and_then(|v| v.as_str()).unwrap_or_default();
    ...
    let Some(album) = crate::immich::access::read_album_as(&client, album_id, &admin).await else {
        return (404, json!({ "error": "that album is gone", "code": "unknown_album" }));
    };
    let assets = read_album_assets_as(&client, album_id, &admin).await.unwrap_or_default();
    let album_name = album
        .get("albumName")
        .and_then(|n| n.as_str())
        .unwrap_or("Shared album")
        .to_string();
    let permissions = if link.get("allowUpload").and_then(|a| a.as_bool()).unwrap_or(false) {
        "contribute"
    } else {
        "view"
    };
    ...
    let album_owner_id = link
        ...
        .and_then(|v| v.as_str())
        .map(str::to_string)
        .or_else(|| album.get("ownerId").and_then(|v| v.as_str()).map(str::to_string))
        .or_else(|| majority_owner(&assets));
    let display_name = match album_owner_id.as_deref() {
        Some(id) => owner_name(&client, id).await.unwrap_or_else(|| cfg().name.clone()),
        None => cfg().name.clone(),
    };
...
fn majority_owner(assets: &[Value]) -> Option<String> {
    ...
    counts.into_iter().max_by_key(|(_, n)| *n).map(|(owner, _)| owner.to_string())
}
...
fn chrono_parse_ms(value: &str) -> Result<i64, ()> {
    ...
    let num = |from: usize, to: usize| -> Result<i64, ()> {
        std::str::from_utf8(&bytes[from..to]).map_err(|_| ())?.parse::<i64>().map_err(|_| ())
    };
    let (y, mo, d) = (num(0, 4)?, num(5, 7)?, num(8, 10)?);
    let (h, mi, s) = (num(11, 13)?, num(14, 16)?, num(17, 19)?);
    let millis = if bytes.len() >= 23 && bytes[19] == b'.' { num(20, 23)? } else { 0 };
...
mod tests {
    ...
    fn an_iso_expiry_parses_to_the_right_instant() {
        ...
        assert_eq!(chrono_parse_ms("2026-01-01T00:00:00.000Z").unwrap(), 1_767_225_600_000);
...
pub fn gone_or_404(state: &crate::state::State, peer_pub: &str, album_mapping_id: &str) -> (u16, Value) {
    ...
    if dead {
        (410, json!({ "error": "this share was withdrawn", "code": "gone" }))
    } else {
...
pub async fn handle_version(caller_pub: &str, album_mapping_id: &str) -> (u16, Value) {
    ...
    let client = Client::new();
    let album = crate::immich::access::read_album_as(&client, &mapping.album_id, &admin_auth()).await;
    let Some(album) = album else {
        return gone_or_404(state, caller_pub, album_mapping_id);
    };
    let updated_at = album.get("updatedAt").and_then(|v| v.as_str()).unwrap_or_default();
    let asset_count = album.get("assetCount").and_then(|v| v.as_i64());
    let comments = client
        .get(&format!("/activities/statistics?albumId={}", mapping.album_id), &admin_auth())
        .await
        .ok()
        .flatten()
...
pub async fn handle_manifest(caller_pub: &str, album_mapping_id: &str) -> (u16, Value) {
    ...
    let client = Client::new();
    let Some(assets) =
        crate::immich::access::read_album_assets_as(&client, &mapping.album_id, &admin_auth()).await
    else {
        return gone_or_404(state, caller_pub, album_mapping_id);
    };
...
pub async fn handle_status(caller_pub: &str, album_mapping_id: &str) -> (u16, Value) {
    ...
    let client = Client::new();
    let updated_at = crate::immich::access::read_album_as(&client, &mapping.album_id, &admin_auth())
        .await
        .and_then(|album| album.get("updatedAt").and_then(|v| v.as_str()).map(str::to_string));
...
pub fn handle_reunified(state: &State, caller_pub: &str, album_mapping_id: &str) -> (u16, Value) {
    ...
    let Some(mapping) = mapping_for(state, &peer.pub_key, album_mapping_id, Some(Role::Owner)) else {
...
pub async fn hello_peers(state: &State) {
    ...
    for peer in peers {
        let header = crate::p2p::frame::RequestHeader { path: "/hello".into(), ..Default::default() };
        let (protocol, features, version) =
            match transport.round_trip(&peer, &header, None).await {
                Ok((head, body)) if head.status < 400 => {
                    ...
                    let version =
                        parsed.get("version").and_then(|v| v.as_str()).map(str::to_string);
        ...
        {
            let mut collections = state.collections();
            if let Some(live) = collections.peers.iter_mut().find(|p| p.pub_key == peer.pub_key) {
...
pub fn handle_nudge(caller_pub: &str, album_mapping_id: &str) -> (u16, Value) {
    let state = crate::state::state();
    let Some(caller) = state.collections().peers.iter().find(|p| p.pub_key == caller_pub).cloned()
    else {
    ...
    let Some(origin) = state.collections().peers.iter().find(|p| p.pub_key == mapping.peer).cloned()
    else {
        return (404, json!({ "error": "unknown album mapping" }));
    };
    ...
    tokio::spawn(async move {
        if mapping.role == Role::Member {
            ...
            if let Err(e) =
                crate::sync::engine::reconcile_mapping(&owned_state, client, &mapping, &origin).await
            {
...
pub async fn handle_refs(caller_pub: &str, album_mapping_id: &str, body: &[u8]) -> (u16, Value) {
    ...
    if mapping.role == Role::Owner && mapping.permissions == "view" {
        return (403, json!({ "error": "view-only album — uploads not allowed", "code": "view_only" }));
    }
    let Ok(parsed) = serde_json::from_slice::<Value>(body) else {
        return (400, json!({ "error": "malformed request" }));
    };
