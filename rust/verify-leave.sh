...
#
#   bash rust/verify-leave.sh [IMMICH_URL] [DATA_DIR]
#
...
rm -rf "$DATA_DIR"
cargo run -q --manifest-path rust/Cargo.toml --example seed_member >/dev/null
cargo run -q --manifest-path rust/Cargo.toml --example leave_probe > /tmp/isa-leave-out.json 2>/tmp/isa-leave-out.log
