...
pub fn offer_window_ms(refreshes_with_no_change: u32) -> i64 {
    let doubled = OFFER_BASE_MS.saturating_mul(
        1i64.checked_shl(refreshes_with_no_change.min(31))
            .unwrap_or(i64::MAX),
    );
...
mod tests {
    ...
    fn the_first_request_with_nothing_seen_before_opens_a_session() {
        assert!(
            opens_session(0, 1_700_000_000_000),
            "a person never seen has never been read"
        );
    }
    ...
    fn a_request_in_the_middle_of_a_session_does_not_read_again() {
        ...
        assert!(
            !opens_session(now - 1_000, now),
            "one second later is the same session"
        );
        assert!(
            !opens_session(now - (SESSION_QUIET_MS - 1), now),
            "one millisecond short is still it"
        );
        assert!(
            opens_session(now - SESSION_QUIET_MS, now),
            "exactly the quiet period counts"
        );
        assert!(
            opens_session(now - SESSION_QUIET_MS * 4, now),
            "hours away is certainly a new one"
        );
    }
    ...
    fn a_new_renamed_grown_or_withdrawn_album_is_a_change() {
        ...
        assert!(
            index_changed(&[album("Summer 2024", 6)], &[album("Summer 2024", 7)]),
            "the count is what a peer prints"
        );
        assert!(index_changed(
            &[album("Summer 2024", 6)],
            &[album("Summer 2024 (edited)", 6)]
        ));
        assert!(
            index_changed(&[album("Summer 2024", 6)], &[]),
            "the peer must stop matching against it"
        );
    }
    ...
    fn a_window_with_no_evidence_of_change_backs_off_and_stops_at_the_cap() {
        assert_eq!(
            offer_window_ms(0),
            OFFER_BASE_MS,
            "the first look after a change is a minute"
        );
        ...
        assert_eq!(
            offer_window_ms(4),
            OFFER_MAX_MS,
            "the cap bounds an idle library"
        );
        assert_eq!(
            offer_window_ms(50),
            OFFER_MAX_MS,
            "and it stays bounded however long the silence"
        );
    }
    ...
    fn a_refresh_is_due_only_once_its_window_has_elapsed() {
        let now = 1_700_000_000_000;
        assert!(
            !should_refresh(now - 1_000, now, 3, now + offer_window_ms(3) - 1),
            "one millisecond short"
        );
        assert!(
            should_refresh(now - 1_000, now, 3, now + offer_window_ms(3)),
            "the window is the whole wait"
        );
    }
