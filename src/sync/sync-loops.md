...
| `album-index.ts` | **The album index itself.** Parses what a person's panel reports (`parsePublishedAlbums` — the request body is a trust boundary, so entries claiming another owner are dropped) and records it per peer for the `/albums` peer route to serve. The sidecar holds no credential for a human and `GET /albums` is scoped to one, so the person's OWN session is the only thing that can enumerate their albums; the panel reads them and this stores the result. |
