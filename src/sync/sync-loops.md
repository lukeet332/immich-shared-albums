...
| `audit.ts` | **The trail, in the album's own comments.** `auditLine(mappingId, albumId, event, text)` posts one line as this household's bot and records it twice in `seen_activity`: `audit:<event>:<albumId>` so a retrying loop cannot write it again, and `local:<activityId>` so `syncCommentsOnce` does not push it back to the peer as though a person had written it. Called where the owner's credential is in hand — adoption — because the bot must be a MEMBER to comment and only an album's owner can grant that. |
