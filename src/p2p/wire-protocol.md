...
| `pair.ts` | Linking two servers on its own, alongside share links. Mints a single-use 15-minute code (the secret rides in the URL **fragment**), and `handlePair` burns it before answering. Signature-bound to the key being enrolled. Conveys **no album access**. |
