...
async fn main() {
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            let header_bytes = match read_frame(&mut stream, HEADER_FRAME_LIMIT)
                .await
                .expect("read header")
            {
                Ok(bytes) => bytes,
            ...
            let body_bytes = match read_frame(&mut stream, body_limit)
                .await
                .expect("read body")
            {
                Ok(bytes) => bytes,
                Err(over) => {
                    ...
                    stream
                        .write_all(&frame(&serde_json::to_vec(&head).unwrap()))
                        .await
                        .unwrap();
